...
torchrun --nproc_per_node=8 --rdzv_conf="timeout=7200" --nnodes=$VC_WORKER_NUM --node_rank=$VC_TASK_INDEX --master_addr $MASTER_ADDR --master_port 12345 train_reward_forcing.py \
    --config_path configs/reward_forcing/reward_forcing_dmd_streaming_noreward_cfg_teleloss.yaml \
    --logdir train_outputs/reward_forcing_dmd_streaming_noreward_cfg_teleloss
