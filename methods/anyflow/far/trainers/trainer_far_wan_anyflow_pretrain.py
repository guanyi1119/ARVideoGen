...
class FAR_Wan_AnyFlow_Pretrain_Trainer(nn.Module, ConfigMixin):

    config_name = 'far_wan_anyflow_pretrain_trainer.json'
    ...
    def __init__(
        ...
        super(FAR_Wan_AnyFlow_Pretrain_Trainer, self).__init__()
        ...
        if transformer_cfg is not None:
            self.transformer = build_model(transformer_cfg['model_type']).from_pretrained(
                ...
                chunk_partition=transformer_cfg['far_config']['chunk_partition'],
                full_chunk_limit=transformer_cfg['far_config']['full_chunk_limit'],
                compressed_patch_size=transformer_cfg['far_config']['compressed_patch_size'],
                subfolder='transformer'
            )

            if not self.transformer.config.get('init_far_model'):
                self.transformer.setup_far_model()
    ...
    def sample_latents(self, latents, far_config):

        if random.random() < far_config['long_context_training_ratio']:
            start_chunkid = self.transformer.config.full_chunk_limit
            end_chunkid = len(far_config['chunk_partition'])
        else:
            start_chunkid = self.transformer.config.full_chunk_limit
            end_chunkid = self.transformer.config.full_chunk_limit

        num_sampled_chunks = torch.tensor([random.choice(range(start_chunkid, end_chunkid + 1))], device=self.config.device)
        torch.distributed.broadcast(num_sampled_chunks, src=0)
        num_frames = sum(far_config['chunk_partition'][:num_sampled_chunks])

        latents = latents[:, :, :num_frames]
        return latents, far_config['chunk_partition'][:num_sampled_chunks]
    ...
    @torch.no_grad()
    def compute_central_difference(self, noisy_latents, latents, noise, context_latents, context_timestep, t, r, prompt_embeds, guidance, chunk_partition):
        def u_func(noisy_latents, t, r):
            return self.transformer(
                torch.cat([context_latents, noisy_latents], dim=1), chunk_partition=chunk_partition,
                timestep=torch.cat([context_timestep, t], dim=1), r_timestep=torch.cat([context_timestep, r], dim=1),
                clean_hidden_states=latents, clean_timestep=torch.zeros_like(t), encoder_hidden_states=prompt_embeds, return_dict=False)
    ...
    def pixel_loss(self, latents, prompt_embeds, iters):
        ...
        causal_loss = self.train_causal(latents, prompt_embeds, iters)
        causal_loss.backward()

        forward_bidirectional = torch.tensor([random.random()], device=self.config.device)
        torch.distributed.broadcast(forward_bidirectional, src=0)

        if forward_bidirectional < self.config.transformer_cfg['bidirectional_modeling_prob']:
            bidirection_loss = self.train_bidirection(latents, prompt_embeds)
            bidirection_loss.backward()
            return {'causal_loss': float(reduce_loss(causal_loss)), 'bidirection_loss': float(reduce_loss(bidirection_loss))}  # noqa: E501
        else:
            return {'causal_loss': float(reduce_loss(causal_loss))}
    ...
    def train_causal(self, latents, prompt_embeds, iters):
        latents, chunk_partition = self.sample_latents(latents, self.config.transformer_cfg['far_config'])

        full_chunk_frame_limit = sum(chunk_partition[-self.transformer.config.full_chunk_limit:])
        context_latents, latents = torch.split(latents, [latents.size(2) - full_chunk_frame_limit, full_chunk_frame_limit], dim=2)
        ...
        # concat context latent
        context_latents = rearrange(context_latents, 'b c t h w -> b t c h w', b=batch_size)
        context_timestep = torch.zeros((batch_size, context_latents.shape[1]), device=latents.device)

        # Predict the noise residual
        noise_pred = self.transformer(
            torch.cat([context_latents, noisy_latents], dim=1), chunk_partition=chunk_partition,
            timestep=torch.cat([context_timestep, t], dim=1), r_timestep=torch.cat([context_timestep, r], dim=1),
            clean_hidden_states=latents, clean_timestep=torch.zeros_like(t), encoder_hidden_states=prompt_embeds, return_dict=False)  # noqa: E501
        ...
        if self.config.text_encoder_cfg.get('fuse_guidance_scale'):
            ...
            with torch.no_grad():
                noise_pred_uncond = self.transformer(
                    torch.cat([context_latents, noisy_latents], dim=1), chunk_partition=chunk_partition,
                    timestep=torch.cat([context_timestep, t], dim=1), r_timestep=torch.cat([context_timestep, r], dim=1),
                    clean_hidden_states=latents, clean_timestep=torch.zeros_like(t), encoder_hidden_states=self.negative_embedding, return_dict=False)
            noise_pred = (noise_pred - (1 - guidance) * noise_pred_uncond) / guidance
        else:
            guidance = 1.0
    ...
    def sample(self, val_dataloader, cfg, global_step=0):
        if hasattr(self, 'ema'):
            self._sample(val_dataloader, cfg, global_step, use_ema=True)
        self._sample(val_dataloader, cfg, global_step, use_ema=False)

    def _sample(self, val_dataloader, cfg, global_step=0, use_ema=True):
        ...
        if use_ema:
            save_root_dir = os.path.join(cfg['path']['visualization'], 'sample', f'iter_{global_step}_ema')
        else:
            save_root_dir = os.path.join(cfg['path']['visualization'], 'sample', f'iter_{global_step}')
        ...
        for item in val_dataloader:
            for num_inference_steps in cfg['val']['sample_cfg']['num_inference_steps']:
                for seed in cfg['val']['sample_cfg']['seed']:
                    ...
                    if 'image' in item:
                        task_type = 'i2v'
                        context_sequence = {'raw': item['image']}
                        context_length = 1
                    elif 'video' in item:
                        task_type = 'v2v'
                        context_sequence = {'raw': item['video']}
