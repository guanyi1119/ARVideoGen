...
"""
SwitchCausalInferencePipeline3Sink — 3-sink parallel variant with prompt switching.

This is a lightweight subclass of SwitchCausalInferencePipeline that
inherits ALL core logic (inference, _recache_after_switch, etc.) unchanged.
...
from methods.reward_forcing.pipelines.switch_causal_inference import (
    SwitchCausalInferencePipeline,
)


class SwitchCausalInferencePipeline3Sink(SwitchCausalInferencePipeline):
    """3-sink parallel variant of SwitchCausalInferencePipeline.
    ...
    def __init__(self, args, device, *, generator=None, text_encoder=None, vae=None):
        ...
        print(
            f"[SwitchCausalInferencePipeline3Sink] 3-sink layout: "
            f"long={long_sink} "
            f"mid={mid_sink} "
            f"rolling={rolling} "
