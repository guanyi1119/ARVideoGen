# CausVid version of causal_model
# Key features vs base (causal_model.py):
#   - Uses window_size instead of local_attn_size/sink_size
#   - Simplified KV cache (direct indexing, not deferred update)
#   - Uses current_end instead of cache_start
#   - No teacher forcing / I2V support
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    @staticmethod
    def _prepare_blockwise_causal_attn_mask_npu(
        device: torch.device | str, num_frames: int = 21,
        frame_seqlen: int = 1560, num_frame_per_block=1
    ) -> BlockMask:
        """
    ...
    def _forward_train(
        ...
        if self.block_mask is None:
            ...
            # )
            self.block_mask = self._prepare_blockwise_causal_attn_mask_npu(
                device, num_frames=x.shape[2],
                frame_seqlen=x.shape[-2] *
                x.shape[-1] // (self.patch_size[1] * self.patch_size[2]),
                num_frame_per_block=self.num_frame_per_block
