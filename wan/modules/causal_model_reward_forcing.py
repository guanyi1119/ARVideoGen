# Reward-Forcing variant of causal_model (cloned from archive/Reward-Forcing/wan/modules/causal_model.py)
# Key differences vs wan/modules/causal_model.py (Causal-Forcing/Self-Forcing version):
#   - CausalWanSelfAttention.__init__ accepts compression_alpha (default 0.999)
#   - Adds `incremental_update` method that performs EMA-style sink token compression on KV cache eviction
#   - KV cache eviction path concatenates [sink_k, k_segment] with dynamic RoPE instead of static overwrite/discard
# These algorithmic differences are required for Reward-Forcing distillation; do NOT merge with the default variant.
# NPU adaptation imports mirror wan/modules/causal_model.py (kept consistent so this module works on both CUDA and NPU).
