# LongLive-RAG variant of causal_model (forked from causal_model_reward_forcing.py)
# Key differences vs causal_model_reward_forcing.py:
#   - CausalWanSelfAttention accepts memory_size (default 0)
#   - CausalWanSelfAttention.forward accepts memory_indices (default None)
#   - When memory_size>0 and memory_indices is provided, the attention output
#     concatenates [k_sink || k_mem || k_local] where k_mem is retrieved from
#     kv_cache["cpu_k_frames"] / ["cpu_v_frames"] by index, then RoPE'd with
#     relative frame index 0 (drops original temporal position).
#   - When memory_indices is None or memory_size == 0, behavior is identical to
#     causal_model_reward_forcing.py (RF's incremental_update EMA sink stays).
#   - KV cache eviction path also saves evicted frames to CPU lists for later retrieval.
# These are required for LongLive-RAG retrieval-augmented inference; do NOT
# merge with the default or longlive variants.
...
class CausalWanSelfAttention(nn.Module):
    ...
    def incremental_update(self, new_k: torch.Tensor, new_v: torch.Tensor,
        ...
        # updated = alpha * current + (1-alpha) * new
    ...
    def forward(
        ...
        if evicted_tokens_exist and evicted_k is not None and evicted_k.shape[1] > 0:
            ...
            # --- LatentMem: save evicted frames to CPU for later retrieval ---
            if self.memory_size > 0:
                num_evicted_frames = evicted_k.shape[1] // frame_seqlen
                if num_evicted_frames > 0:
                    ev_k = evicted_k.view(b, num_evicted_frames, frame_seqlen, n, d).split(1, dim=1)
                    ev_v = evicted_v.view(b, num_evicted_frames, frame_seqlen, n, d).split(1, dim=1)
                    kv_cache.setdefault("cpu_k_frames", []).extend(
                        [f.clone().detach().to("cpu", non_blocking=True) for f in ev_k]
                    )
                    kv_cache.setdefault("cpu_v_frames", []).extend(
                        [f.clone().detach().to("cpu", non_blocking=True) for f in ev_v]
                    )

        # === ATTENTION COMPUTATION ===

        if self.memory_size > 0 and memory_indices is not None:
            # Memory retrieval path: concatenate [sink || mem || local]
            local_budget = self.max_attention_size - total_sink_tokens - (self.memory_size * frame_seqlen)

            # --- Sink tokens ---
            k_sink = kv_cache["k"][:, :total_sink_tokens]
            v_sink = kv_cache["v"][:, :total_sink_tokens]
            sink_grid = copy.deepcopy(grid_sizes)
            sink_grid[:, 0] = self.sink_size
            k_sink_roped = causal_rope_apply(k_sink, sink_grid, freqs, start_frame=0).type_as(v)

            # --- Memory tokens from CPU storage ---
            k_mem_roped = None
            v_mem = None
            ...
            if len(cpu_k_list) > 0:
                ...
                for bi in range(b):
                    ...
                    for k_idx in indices:
                        k_mem_unroped_list.append(cpu_k_list[k_idx][bi, 0].to(device_k, non_blocking=True))
                        v_mem_list.append(cpu_v_list[k_idx][bi, 0].to(device_k, non_blocking=True))
                ...
                k_mem_roped = causal_online_rope(
                    k_mem_unroped, mem_grid, freqs,
                    relative_frame_indices=torch.zeros(k_sel, dtype=torch.long, device=device_k)
                ).type_as(v)

            # --- Local tokens ---
            local_start_for_window = max(total_sink_tokens, local_end_index - local_budget)
            k_local = kv_cache["k"][:, local_start_for_window:local_end_index]
            v_local = kv_cache["v"][:, local_start_for_window:local_end_index]
            local_tokens = k_local.shape[1]
            local_frames = local_tokens // frame_seqlen
            local_grid = copy.deepcopy(grid_sizes)
            local_grid[:, 0] = local_frames
            k_local_roped = causal_rope_apply(k_local, local_grid, freqs, start_frame=0).type_as(v)

            # --- Combine ---
            k_parts = [k_sink_roped]
            v_parts = [v_sink]
            if k_mem_roped is not None:
                k_parts.append(k_mem_roped)
                v_parts.append(v_mem)
            if local_tokens > 0:
                k_parts.append(k_local_roped)
            ...
            x = attention(
                causal_rope_apply(q, grid_sizes, freqs, start_frame=query_start_frame).type_as(v),
                k_cat,
                v_cat,
            )

        elif self.sink_size > 0:
            # Original RF sink path (unchanged)
            ...
            total_combined_tokens = total_sink_tokens + (kv_end_index - kv_start_index)
            ...
            # Original RF no-sink path (unchanged)
            kv_start_index = max(total_sink_tokens, local_end_index - self.max_attention_size + total_sink_tokens)
            kv_end_index = local_end_index
    ...
    def __init__(self,
        ...
        Args:
            ...
            memory_size (`int`, *optional*, defaults to 0):
                Number of memory frames for retrieval-augmented inference (LongLive-RAG).
                When > 0, evicted frames are saved to CPU and can be retrieved via memory_indices.
