# LongLive-RAG inference entry point
# Forked from inference_reward_forcing_streaming.py
# Uses LatentMemCausalInferencePipeline instead of CausalInferencePipeline
...
from methods.longlive_rag.pipelines import LatentMemCausalInferencePipeline
...
parser.add_argument("--global_sink", type=lambda x: x.lower() == "true",
                    default=True, help="Use global sink (default: True)")
...
parser.add_argument("--enlarged_cfg_scale", type=float, default=0.0,
                    help="Enlarged CFG scale for inference (0.0=disabled, "
                         "1.0=standard).")
args_cli = parser.parse_args()

default_config_path = os.path.join(
    os.path.dirname(args_cli.config_path), "default_config.yaml"
)
...
# Initialize LatentMemCausalInferencePipeline (RAG-aware)
pipeline = LatentMemCausalInferencePipeline(config, device=device)
    ...
    if idx < num_prompts:
        ...
        for seed_idx in range(config.num_samples):
            if config.save_with_index:
                ...
                output_path = os.path.join(
                    config.output_folder, f"{name}.mp4"
                )
