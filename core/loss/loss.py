...
class X0PredLoss(DenoisingLoss):
    def __call__(
        ...
        alphas_cumprod: torch.Tensor,
        timestep: torch.Tensor,
        gradient_mask: torch.Tensor = None,  # [DIFF-Self-Forcing/LongLive] Added gradient_mask support
        **kwargs
    ) -> torch.Tensor:
        err = (x - x_pred) ** 2
        # [DIFF-Causal-Forcing] Causal-Forcing version does not support gradient_mask
