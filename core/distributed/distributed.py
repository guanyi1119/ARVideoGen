...
class EMA_FSDP:
    """Exponential Moving Average for FSDP-wrapped models.

    Merges Causal-Forcing and Self-Forcing implementations:
    - Uses Self-Forcing's FSDP.summon_full_params() pattern in _init_shadow and update
      [DIFF-Causal-Forcing] Causal-Forcing accessed fsdp_module.module.named_parameters() directly
    - Uses Causal-Forcing's copy_to pattern (direct parameter assignment)
      [DIFF-Self-Forcing] Self-Forcing used summon_full_params(writeback=True) in copy_to
    - Includes Causal-Forcing's full_state_dict() method for checkpoint saving
    """
    ...
    def _init_shadow(self, fsdp_module):
        # [DIFF-Self-Forcing] Uses summon_full_params to ensure all params are gathered
    ...
    def copy_to(self, fsdp_module):
        # [DIFF-Causal-Forcing] Direct parameter assignment without summon_full_params
        for n, p in fsdp_module.module.named_parameters():
            if n in self.shadow:
