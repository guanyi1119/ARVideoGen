# LongLive version of wan_wrapper
# Key features vs base (wan_wrapper.py):
#   - WanDiffusionWrapper: sink_recache_after_switch param, dynamic seq_len (1560*local_attn_size when > 21),
#     use_infinite_attention flag with CausalWanModelInfinity, passes sink_recache_after_switch in all forward calls
#   - WanVAEWrapper: adds decode_to_pixel_chunk for chunked decoding to avoid OOM on long videos
#   - WanTextEncoder: moves text encoder to GPU immediately in __init__
#   - [DIFF-Base] Base (CF/SF) lacks sink_recache_after_switch, uses fixed seq_len=32760, no chunked decode
#   - [DIFF-CausVid] CausVid uses DiffusionModelInterface, current_end, returns pred_x0 only, no clean_x/classify_mode
import os
import types
