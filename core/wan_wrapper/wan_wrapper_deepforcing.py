# DeepForcing version of wan_wrapper
# Key features vs other variants:
#   - WanDiffusionWrapper: adds is_ds_only, budget, recent parameters, supports CausalWanModelDS
...
from wan.modules.causal_model_deepforcing import CausalWanModel
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            # Check if we have CausalWanModelDS available (DeepForcing specific)
            try:
                from wan.modules.causal_model_deepforcing_DS import CausalWanModelDS
                if is_ds_only:
                ...
                # If CausalWanModelDS is not available, fall back to standard CausalWanModel
                self.model = CausalWanModel.from_pretrained(
                    f"{os.environ.get('MODELS_DIR', 'wan_models')}/{model_name}/",
                    local_attn_size=local_attn_size,
                    sink_size=sink_size)
