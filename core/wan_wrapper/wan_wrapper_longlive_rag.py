# LongLive-RAG variant of wan_wrapper
# Forked from core/wan_wrapper/wan_wrapper_reward_forcing.py with the following changes:
#   - Imports CausalWanModel from wan.modules.causal_model_latentmem instead of
#     wan.modules.causal_model_reward_forcing
#   - WanDiffusionWrapper.__init__ accepts memory_size=0 and passes it to
#     CausalWanModel construction. Additional optional config knobs
#     (recent_exclude, use_latentmem, compression_method, ae_ckpt) are stored
#     on self for introspection by the inference pipeline.
#   - WanDiffusionWrapper.forward accepts memory_indices=None and passes it to
#     self.model(...).
# All other logic is identical to the Reward-Forcing wrapper (see that file's
# header for details).
import os
import types
...
from core.scheduler.scheduler import SchedulerInterface, FlowMatchScheduler
from wan.modules.tokenizers import HuggingfaceTokenizer
from wan.modules.model import WanModel, RegisterTokens, GanAttentionBlock
from wan.modules.vae import _video_vae
from wan.modules.t5 import umt5_xxl
from wan.modules.causal_model_latentmem import CausalWanModel
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"{os.environ.get('MODELS_DIR', 'wan_models')}/{model_name}/",
                local_attn_size=local_attn_size, sink_size=sink_size,
                memory_size=memory_size)
        else:
            self.model = WanModel.from_pretrained(f"{os.environ.get('MODELS_DIR', 'wan_models')}/{model_name}/")
        self.model.eval()

        # Store config knobs for inference pipeline introspection
        self.recent_exclude = recent_exclude
        self.use_latentmem = use_latentmem
    ...
    def forward(
        ...
        memory_indices: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
