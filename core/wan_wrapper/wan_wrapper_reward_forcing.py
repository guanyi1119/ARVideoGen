# Reward-Forcing variant of wan_wrapper
# Cloned from core/wan_wrapper/wan_wrapper.py with a single change: it loads CausalWanModel from
# wan.modules.causal_model_reward_forcing instead of wan.modules.causal_model, because Reward-Forcing
# requires the EMA sink-token compression in the cache eviction path.
# All other logic is identical to the Causal-Forcing/Self-Forcing wrapper (see that file's header for details).
import os
import types
...
from wan.modules.causal_model_reward_forcing import CausalWanModel
