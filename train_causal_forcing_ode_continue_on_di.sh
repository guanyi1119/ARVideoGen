...
python -c "import moxing as mox; mox.file.copy_parallel('obs://yw-pixelgeek-training-data-gy1/01.USERS/z00546255/data/custom_ct_2m_shrink/test_lmdb_ode', '/cache/test_lmdb_ode')"
...
# AR Diffusion model should be put here: /cache/init_model.pt
...
torchrun --nproc_per_node=8 --rdzv_conf="timeout=7200" --nnodes=$VC_WORKER_NUM --node_rank=$VC_TASK_INDEX --master_addr $MASTER_ADDR --master_port 12345 train_causal_forcing.py \
    ...
    --data_path /cache/test_lmdb_ode \
    --data_name_pattern test_lmdb_ode
