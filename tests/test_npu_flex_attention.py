...
def npu_flex_attention_v2(query, key, value, block_mask=None):
    ...
    Same signature as flex_attention(query, key, value, block_mask).
    Kept in sync with wan.modules.causal_model.npu_flex_attention_v2.
    """
...
def npu_flex_attention_v2_sdpa_fallback(query, key, value, block_mask=None):
    ...
    # Strip 128-alignment padding
    q_real = query[:, :, :real_L]
    k_real = key[:, :, :real_L]
    v_real = value[:, :, :real_L]

    # Convert BNSD [1, H, real_L, D] to TND [real_L, H, D] for slicing
    q_tnd = q_real[0].permute(1, 0, 2)
    k_tnd = k_real[0].permute(1, 0, 2)
    v_tnd = v_real[0].permute(1, 0, 2)
    ...
    for i in range(num_blocks):
        ...
        seg_q = q_tnd[q_start:q_end].unsqueeze(0).transpose(1, 2)
        seg_k = k_tnd[:kv_end].unsqueeze(0).transpose(1, 2)
        seg_v = v_tnd[:kv_end].unsqueeze(0).transpose(1, 2)

        seg_out = F.scaled_dot_product_attention(seg_q, seg_k, seg_v)
        seg_outputs.append(seg_out[0].permute(1, 0, 2))
    ...
    for i in range(num_blocks):
        ...
        seg_q = q_tnd[q_start:q_end].unsqueeze(0).transpose(1, 2)
        if context_len > 0:
            seg_k = torch.cat([k_tnd[:context_len], k_tnd[q_start:q_end]], dim=0).unsqueeze(0).transpose(1, 2)
            seg_v = torch.cat([v_tnd[:context_len], v_tnd[q_start:q_end]], dim=0).unsqueeze(0).transpose(1, 2)
        else:
            seg_k = k_tnd[q_start:q_end].unsqueeze(0).transpose(1, 2)
            seg_v = v_tnd[q_start:q_end].unsqueeze(0).transpose(1, 2)

        seg_out = F.scaled_dot_product_attention(seg_q, seg_k, seg_v)
        seg_outputs.append(seg_out[0].permute(1, 0, 2))

    # Concat and reshape: [real_L, H, D] -> [1, H, real_L, D]
    attn_out_tnd = torch.cat(seg_outputs, dim=0)
    attn_out = attn_out_tnd.reshape(1, real_L, H, D).permute(0, 2, 1, 3)
