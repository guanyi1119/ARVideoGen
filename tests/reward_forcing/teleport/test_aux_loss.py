...
class TestFactory:
    ...
    def test_factory_missing_threshold_raises(self):
        with pytest.raises(ValueError, match="requires explicit threshold"):
            build_aux_loss({"type": "masked_mean"})

    def test_factory_masked_mean_with_threshold(self):
        loss = build_aux_loss({"type": "masked_mean", "threshold": 0.3})
        assert isinstance(loss, MaskedMeanAuxLoss)
        assert loss._threshold == 0.3
        assert loss._aggregation == "masked_mean"

    def test_factory_masked_mean_with_aggregation_mean(self):
        loss = build_aux_loss(
            {"type": "masked_mean", "threshold": 0.2, "aggregation": "mean"}
        )
        assert isinstance(loss, MaskedMeanAuxLoss)
        assert loss._threshold == 0.2
...
class TestConstructor:
    def test_constructor_threshold_none_raises(self):
        with pytest.raises(ValueError, match="requires explicit threshold"):
            MaskedMeanAuxLoss(threshold=None)

    def test_constructor_invalid_aggregation_raises(self):
        with pytest.raises(ValueError, match="aggregation must be"):
            MaskedMeanAuxLoss(threshold=0.1, aggregation="bogus")

    def test_constructor_default_aggregation(self):
        loss = MaskedMeanAuxLoss(threshold=0.5)
        assert loss._aggregation == "masked_mean"
...
class TestMaskedMean:
    def test_masked_mean_zero_score_returns_zero_grad_able(self):
        ...
        loss_fn = MaskedMeanAuxLoss(threshold=0.3)
    ...
    def test_masked_mean_score_above_threshold_contributes(self):
        """Score values > threshold should produce non-zero loss."""
        loss_fn = MaskedMeanAuxLoss(threshold=0.3)
    ...
    def test_masked_mean_score_below_threshold_excluded(self):
        """All scores below threshold → mask is all zeros → L_aux == 0.0."""
        loss_fn = MaskedMeanAuxLoss(threshold=0.5)
        score = _make_score_map_with_patch(
    ...
    def test_masked_mean_correct_formula(self):
        ...
        loss_fn = MaskedMeanAuxLoss(threshold=0.3)
        # Manual: values [0.1, 0.2, 0.4, 0.5], threshold=0.3
    ...
    def test_masked_mean_mixed_above_below(self):
        """Some pixels above threshold, some below — only above contribute."""
        loss_fn = MaskedMeanAuxLoss(threshold=0.3)
...
class TestMeanAggregation:
    def test_mean_aggregation_no_mask(self):
        ...
        loss_fn = MaskedMeanAuxLoss(threshold=0.3, aggregation="mean")
...
class TestAbstract:
    ...
    def test_subclass_isinstance(self):
        loss = MaskedMeanAuxLoss(threshold=0.3)
